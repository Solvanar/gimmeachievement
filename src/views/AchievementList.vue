...
<template>
  <div class="list-page">
    ...
    <p v-else-if="store.error" class="state-msg state-error">{{ store.error }}</p>
...
.list-header h1 {
  ...
  margin: 0 0 8px;
  background: linear-gradient(135deg, var(--hero-text-from) 0%, var(--hero-text-to) 100%);
...
@media (max-width: 480px) {
  .list-page { padding: 40px 16px; row-gap: 32px; }
  .cards-grid {
    grid-template-columns: 1fr;
    column-gap: 0;
    row-gap: 32px;
